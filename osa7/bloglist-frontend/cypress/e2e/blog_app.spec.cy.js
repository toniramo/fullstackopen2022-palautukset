describe('Blog app', function () {
  ...
  describe('When logged in', function () {
    ...
    it('A blog can be created', function () {
      cy.get('#blogs').should('not.contain', 'Uusi ploki');
      ...
      cy.get('.info')
        .should(
          'contain',
          'A new blog Uusi ploki by Tuntematon kirjoittaja added.'
        )
        .and('have.css', 'color', 'rgb(0, 128, 0)')
        .and('have.css', 'border-style', 'solid');

      cy.get('#blogs').should('contain', 'Uusi ploki');
    });
    ...
    describe('and some blogs exists', function () {
      ...
      it('blog can be liked', function () {
        ...
        cy.get(`@${blog1.title}`).contains(`likes: ${blog1.likes + 1}`);
      });

      it('blog can be removed by user that has added it', function () {
        cy.viewBlog(blog2.title).contains('button', 'Remove').click();

        cy.get('#blogs').should('not.contain', blog2.title);
      });
      ...
      it('blogs are ordered based on likes', function () {
        ...
        cy.viewBlog(blog3.title).contains('button', 'Like').click().click();
